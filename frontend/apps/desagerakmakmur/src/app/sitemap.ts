...
import { getArticles, getUmkmArticles } from "@/data/articles";
 ...
 * seluruh halaman publik (termasuk artikel berita & profil UMKM yang
 * jumlahnya bertambah dari admin) ditemukan dan diindeks, bukan cuma
 * beranda.
 */
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [berita, umkm] = await Promise.all([
    getArticles().catch(() => []),
    getUmkmArticles().catch(() => []),
  ]);
  const halamanArtikel: MetadataRoute.Sitemap = [...berita, ...umkm].map(
    (a) => ({
