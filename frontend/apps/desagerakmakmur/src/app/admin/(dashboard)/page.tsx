...
export default function AdminOverviewPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("umkm").select("id, is_published").eq("village_id", vid),
      supabase
        .from("villages")
        .select(
    ...
    ]).then(([spots, pakets, galeri, artikel, umkm, desa, struktur, statistik, dokumen]) => {
      const firstError =
        spots.error ?? pakets.error ?? galeri.error ?? artikel.error ?? umkm.error;
      ...
      setStats({
        ...
        umkm: { total: umkm.data?.length ?? 0, published: pub(umkm.data) },
      });
    });
  }, [admin.village.id]);
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 font-body text-3xl font-bold leading-tight tracking-tight text-[#006572]">
            Ringkasan
          </h1>
          <p className="mt-1 font-body text-sm text-[#006572]/60">
      ...
      {pendingGaleri > 0 && (
        <Card className="border-[#006572]/30 bg-accent">
          <CardContent className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <ImagesIcon className="size-5 shrink-0 text-[#006572]" />
        ...
        {STAT_CARDS.map(({ key, label, href, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href} className="group no-underline">
              <Card className="gap-2 py-5 motion-safe:transition-colors group-hover:border-[#006572]/50">
                <CardHeader className="flex-row items-center justify-between px-5">
                  ...
                  <Icon className="size-4 text-[#006572]/50" />
                </CardHeader>
                <CardContent className="px-5">
                  <p className="font-body text-3xl font-bold tracking-tight text-[#006572] tabular-nums">
                    {stat?.total ?? "…"}
                  </p>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="font-body text-lg font-bold tracking-tight text-[#006572]">
              Artikel Terbaru
            </CardTitle>
            <CardDescription className="font-body text-[#006572]/60">
              Berita dan profil UMKM yang terakhir diperbarui
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
                        ...
                        <span className="inline-flex items-center gap-2 font-body text-sm text-foreground">
                          <span
                            className={`size-1.5 rounded-full ${
                              a.is_published ? "bg-[#006572]" : "bg-outline"
                            }`}
                          />
          ...
          <CardFooter>
            <Button asChild variant="ghost" size="sm" className="text-[#006572]">
              ...
              <div className="flex flex-col">
                ...
                >
                  <span className="flex items-center gap-3">
                    <LandmarkIcon className="size-4 text-[#006572]" />
                  ...
                  <span className="mt-2 block h-1 overflow-hidden rounded-full bg-surface-container-high">
                    <span
                      className="block h-full rounded-full bg-[#006572] motion-safe:transition-[width] motion-safe:duration-300"
                ...
                {(
                  ...
                  >
                    <Icon className="size-4 text-[#006572]" />
                    <span className="flex-1 font-body text-sm font-semibold text-foreground">
                      {label}
                    </span>
                    <span className="font-body text-sm text-muted-foreground tabular-nums">
